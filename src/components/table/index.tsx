...
function GenericTable<T>({
  ...
  const renderCellContent = (row: Row<T>, key: string): ReactNode => {
    ...
    if (typeof value === 'string' || typeof value === 'number' || typeof value === 'boolean') {
      ...
      return (
        <span
          className={`table-cell ${columnType === "currency" || pivotColumnType === "currency" ? 'table-cell-currency' : ''}`}
          style={{
            justifyContent: getCellValueAlignment(columnType),
          }}
        >
    ...
    return (
      <div className="no-data-container">
        <div className="no-data-box">
          {isLoading ? <div style={{ color: 'red' }}>Loading...</div> : <div>No data</div>}
        </div>
      </div>
    );
  }
          ...
          <TableHead>
            <TableRow className="table-header-row">
              {meta.chartType === TABLE_TYPES.MULTI_LEVEL && (
                <TableCell
                  className={`table-header-cell ${depth === 1 ? 'table-header-cell-first-level' : ''}`}
                  sx={{
                    ...DEFAULT_TABLE_HEADER_STYLES,
                    ...tableHeaderStyles,
                  }}
                >
                  ...
                  >
                    <ArrowDropDownIcon
                      className={`expand-icon ${!allExpanded ? 'expand-icon-collapsed' : ''}`}
                    />
                  </button>
                </TableCell>
              )}
              {columns.map((column, i) => (
                <TableCell
                  ...
                  className={`
                    ${i === columns.length - 1 && !isShowActionColumn ? 'border-radius-top-right border-radius-bottom-right' : ''}
                    ${meta.chartType !== "MULTI_LEVEL_TABLE" && i === 0 ? 'border-radius-top-left border-radius-bottom-left' : ''}
                  `}
                  sx={{
